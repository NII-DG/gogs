...
func Test_dmpUtil_fetchDmpSchema(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if !tt.wantErr {
				mockFileUtil.EXPECT().GetFileBypath(filepath.Join(tt.path, "dmp", "json_schema", tt.orgName)).Return("OK", nil)
				tt.d.fileUtil = mockFileUtil
			} else if tt.wantErr {
				mockFileUtil.EXPECT().GetFileBypath(filepath.Join(tt.path, "dmp", "json_schema", tt.orgName)).Return("", fmt.Errorf("NG"))
				tt.d.fileUtil = mockFileUtil
			}
