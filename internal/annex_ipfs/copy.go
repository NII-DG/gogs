...
func CopyFromByKey(remote, key, repoPath string) error {
...
//
//@param repoPath string　実行レポジトリパス
func CopyFrom(remote, repoPath string) error {
...
//git annex copy --to <reote> 実行メソッド
//
//@param remote string 指定リモート（github, ipfs ....）
//
//@param repoPath string　実行レポジトリパス
func CopyTo(remote, repoPath string) error {
...
//git annex copy --to <reote> --key <key>　実行メソッド
//
//@param remote string 指定リモート（github, ipfs ....）
//
//@param key string Annexキー
//
//@param repoPath string　実行レポジトリパス
func CopyToByKey(remote, key, repoPath string) error {
	cmd := git.NewCommand("annex", "copy", "--to", remote, "--key", key)
	if _, err := cmd.RunInDir(repoPath); err != nil {
		return fmt.Errorf("[Failure git annex copy to %v] err : %v , fromPath : %v, key : %v", remote, err, repoPath, key)
	}
	return nil
}
